...
async function chargerDossier(idPatient, { notesVisiblesSeulement = false } = {}) {
    const [[identite], etapes, praticiens, seances, notes, [questionnaire]] = await Promise.all([
        ...
        pool.query(
            `SELECT pr.id_utilisateur AS id, u.prenom, u.nom, pr.specialite
...
router.get('/patients/:id', praticienSeulement, route(async (req, res) => {
    ...
    const dossier = await chargerDossier(id);
