...
export default function ListePatients() {
  ...
  return (
    <>
      <Bande ton="creme">
        <Pastille>Espace praticien</Pastille>
        ...
        <div className="mt-8 flex flex-wrap gap-5">
          <Tuile ton="blanche" valeur={tous ? tous.length : '–'} libelle="patients suivis" inclinaison={-2}
            onClick={() => setFiltres((f) => ({ ...f, statut: '' }))} actif={filtres.statut === ''} />
          <Tuile ton="bleu" valeur={compte('a_demarrer')} libelle="à démarrer" inclinaison={1.5}
            onClick={() => filtrerStatut('a_demarrer')} actif={filtres.statut === 'a_demarrer'} />
          <Tuile ton="orange" valeur={compte('en_cours')} libelle="en cours" inclinaison={-1}
            onClick={() => filtrerStatut('en_cours')} actif={filtres.statut === 'en_cours'} />
          <Tuile ton="vert" valeur={compte('termine')} libelle="terminés" inclinaison={2}
      ...
      <Bande ton="sable">
        <Carte>
          ...
          {patients === null && !erreur && <p role="status" className="min-h-96">Chargement…</p>}
                    ...
                    <tbody role="rowgroup">
                      {patients.map((p) => (
                        <tr key={p.id} role="row" className="hover:bg-base-200">
                          ...
                          <td role="cell" data-label="Étape en cours">{p.etapeEnCours ?? '—'}</td>
                          ...
                          <td role="cell" data-label="Progression">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="whitespace-nowrap text-sm">{p.etapesRealisees}/{p.etapesTotal} étapes</span>
                            </div>
                          </td>
                          <td role="cell" data-label="Prochaine séance">{p.prochaineSeance ? dateHeure(p.prochaineSeance) : '—'}</td>
